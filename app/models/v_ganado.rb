class VGanado < ActiveRecord::Base
  ...
  scope :modulo_ganado_salida, -> { where("estado_ganado_id not in (?)", [PARAMETRO[:estado_ganado_en_proceso_venta], PARAMETRO[:estado_ganado_vendido], PARAMETRO[:estado_ganado_muerto]])}
